...
async function testAntiBan() {
  ...
  try {
    ...
    for (let i = 0; i < 3; i++) {
      const delay = antiBan.getRandomDelay();
    ...
    antiBan.recordMessageSent('test_tenant');
    const inCooldown = antiBan.isInCooldown('test_tenant');
    log('ℹ️', `En cooldown: ${inCooldown}`, colors.cyan);
    
    // Test 4: Estadísticas
    log('\n📋', 'Paso 4: Obteniendo estadísticas...');
    const stats = antiBan.getStats('test_tenant');
...
async function testIntegration() {
  ...
  try {
    ...
    for (let i = 0; i < 10; i++) {
      if (antiBan.canSendMessage(tenantId)) {
        // Simular delay
        const delay = antiBan.getRandomDelay();
        await new Promise(resolve => setTimeout(resolve, delay));
        
        antiBan.recordMessageSent(tenantId);
        successCount++;
    ...
    const stats = antiBan.getStats(tenantId);
    log('ℹ️', `Tasa de envío: ${stats.messagesPerMinute.toFixed(2)} msg/min`, colors.cyan);
