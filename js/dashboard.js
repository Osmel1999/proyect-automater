...
let menuItems = [];
let onboardingState = {
  whatsapp_connected: false, // Ahora empieza en false, se actualiza al verificar conexión real
  menu_configured: false,
  messages_customized: false,
  bot_activated: false // Cambiado de bot_tested a bot_activated
};
...
async function checkWhatsAppStatus() {
  try {
    ...
    // Actualizar onboardingState con el estado real de WhatsApp
    const wasConnected = onboardingState.whatsapp_connected;
    onboardingState.whatsapp_connected = data.connected === true;
    ...
    onboardingState.whatsapp_connected = false;
  ...
  try {
    ...
    if (!tenantData) {
      ...
      tenantData = {
        ...
        onboarding: { steps: onboardingState, progress: 0 }
      };
    }
    ...
    // Mostrar el control del bot SOLO si el onboarding está completo
    if (isCompleted) {
      document.getElementById('bot-control-container').style.display = 'block';
      updateBotControlUI();
      showCompletionScreen();
    } else {
      document.getElementById('bot-control-container').style.display = 'none';
      showWizard();
    }

  } catch (error) {
    console.error('Error loading tenant:', error);
    // Solo mostrar error si es crítico (no simplemente datos vacíos)
    document.getElementById('loading-container').style.display = 'none';
    
    // Mostrar el wizard de todas formas para que el usuario pueda configurar
    showWizard();
  }
}
...
async function activateBotStep() {
  ...
  try {
    ...
    botActive = true;
    onboardingState.bot_activated = true;
    await saveOnboardingState();
    
    alert('🎉 ¡Felicidades! Tu bot está activo y listo para recibir pedidos.');
...
// ====================================
// TEST BOT (deprecated - kept for compatibility)
// ====================================
...
function updateBotControlUI() {
  ...
  if (!toggle || !label || !statusText || !card) {
    return;
  }
  ...
  if (botActive) {
    ...
    if (statusLabel) statusLabel.textContent = 'Bot activo y respondiendo mensajes';
    ...
    if (statusLabel) statusLabel.textContent = 'Bot desactivado, no responderá mensajes';
